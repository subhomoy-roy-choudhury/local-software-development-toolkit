...
class LocalSolr(object):
    ...
    def reload_solr_core(self, core: str):
        self._request_solr(
            ...
            params={"action": "RELOAD", "core": core, "wt": "json"},
        )
        return 0
    ...
    def setup_products_core(self, core: str):
        self.docker_client.copy_to_container(
            "solr_config/products/managed-schema",
            f"local-solr:/var/solr/data/{core}/conf/",
        )
        self.docker_client.copy_to_container(
            "solr_config/products/currency.xml",
            f"local-solr:/var/solr/data/{core}/conf/",
        )
        self.docker_client.copy_to_container(
            "solr_config/products/data-config.xml",
            f"local-solr:/var/solr/data/{core}/conf/",
        )
    ...
    def exec(self):
        ...
        solr_engine = pysolr.Solr(
            f"{self.solr_base_url}solr/{core}", use_qt_param=False, verify=False
        )
