...
container_id=$(docker ps -aqf "name=${CC}");    # var MONGO_CONTAINER_NAME is from loca.env
...
function get_db() {
    docker exec -it $MONGO_CONTAINER_NAME bash -c "rm -rf ${1} && unzip -o db_zip/${1}.zip && mongorestore --db ${1} --gzip ${1}"
}
